...
function extractPlats(html, title) {
...
async function fetchMenusFromExternalAPI() {
    try {
        ...
        const restoR135 = restaurants.find((resto) => resto.$.id === ru_lumiere_id);
        const menus = decodeJsonValues(restoR135.menu);
        const transformedMenus = menus.map((menu) => transformToMenu(menu));
